...
export default async function Page(props: {
    ...
    const [t, outGroupData, outGroupTotalPages, inGroupData, inGroupTotalPages] = await Promise.all([
        ...
        group === "user"
            ? fetchUsersNotInGroup(itemsPerPage, currentOutPage)
            : group === "device"
                ? fetchDevicesNotInGroup(itemsPerPage, currentOutPage)
                : fetchDeptsNotInGroup(itemsPerPage, currentOutPage),
        group === "user"
        ...
        group === "user"
            ? fetchUsersInGroup(id, itemsPerPage, currentOutPage)
            : group === "device"
                ? fetchDevicesInGroup(id, itemsPerPage, currentOutPage)
                : fetchDeptsInGroup(id, itemsPerPage, currentOutPage),
        group === "user"
            ? fetchUsersInGroupPages(id, itemsPerPage)
            : group === "device"
                ? fetchDevicesInGroupPages(id, itemsPerPage)
                : fetchDeptsInGroupPages(id, itemsPerPage),
    ]);
